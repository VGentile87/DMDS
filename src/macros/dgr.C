...
void bfcl(int iv, int igr, int dr, int cl0,int cl1,int cl2,int cl3,int cl4,int cl5,int cl6,int cl7)
{
  ...
  for(int i=0;i<8;i++){
    ...
    DMRImageCl *im    = run->GetCLIMBFC(iv,cl_id[i],i,dr,x0,y0);
    if(im){
  ...
  gxy2->SetName("xy_bar");
   for(int k=0;k<8;k++){
     cout << "points " << k << " " << xcp2[k] << " " << ycp[k]  << endl;
     if(!rmv_pnt[k]){
...
void mtrk(int iv, int imt, int igr1, int igr2, int dr)
{
  float pixX = run->GetHeader()->pixX;
  float pixY = run->GetHeader()->pixY;
  int nx     = run->GetHeader()->npixX;
  ...
  for(int i=0;i<2;i++){
    //if(cl_id[i]!=-1){
      DMRImageCl *im    = run->GetGRIM(iv,gr_id[i],0,dr);
      //DMRCluster *cl = v->GetCL(gr_ibfcgr_id[i]);
      if(im){
	TObjArray  *acl = run->GetGRCLs( iv,gr_id[i],0,dr );  //clusters
	//h[i] = im->GetHist2();
	//if(h[i]->GetMaximum()>tmp_max)tmp_max = h[i]->GetMaximum();
	//if(h[i]->GetMinimum()<tmp_min)tmp_min = h[i]->GetMinimum();
...
TH2F* drawimMT(DMRImageCl *im, TObjArray  *acl)
{
  TH2F *h =0;
  if(im)
  {
...
void drawMT( DMRCluster *cl)
{
  ...
  //if(cl->igr[cl->ID()]==igr) m->SetMarkerColor(2);
...
double scaling(int iv, int icl, int ipol)
{

  int dr=50;
  TH2F *h;
  TH1F *hspect = new TH1F("hspect","hs",255,0,255); 
  DMRImageCl *im    = run->GetCLIM(iv,icl,ipol,dr);
  if(im){
    h = im->GetHist2();
  }
...
void Nbfcl(int ifrom, int ntimes)
{
  ...
  for(int i =0; i< runDescription.size();i++){
    ...
    for(int j = 0; j<nstring; j++){
       char *text = runDescription[i][j].c_str();
